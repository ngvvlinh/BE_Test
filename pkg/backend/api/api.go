...
func NewServiceApi(s umodel.IController, logger log.ILogger) *BackendApi {
	...
	api_ufyler := e.Group("/ufyler/")
	{
		...
		api_ufyler.GET("v1", apiUfylerV1GetObj(s, uf))
		api_ufyler.PUT("v1", apiUfylerV1PutObj(s, uf))
...
func (c *BackendApi) Run(server *http.Server) error {
...
func (c *BackendApi) Routes() []string {
	var routes []string
