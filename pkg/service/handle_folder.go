...
func (s *Controller) readddir(name string, limit int) ([]os.FileInfo, error) {
	...
	st = st[:num]
	sort.Sort(FileInfoByName(st))

	return st, err
}

func (s *Controller) walkdir(name string, limit int) ([]FileInfo, error) {
	...
	if err = afero.Walk(s.fs, _name, walker); err != nil && err != ErrSizeLimit {
		return nil, err
	}
...
func (s *Controller) copier(srcFs, dstFs afero.Fs, name, name2 string) (int, []*os.PathError, error) {
	...
	walker := func(path string, fi os.FileInfo, err error) error {
		...
		if fi.IsDir() {
			...
			if err := s.fileCopy(srcFs, dstFs, path, path); err != nil {
